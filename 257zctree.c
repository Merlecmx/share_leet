 ...
 */
    ...
    if (root->left == NULL && root->right == NULL) {
        ...
        for (i = 0; i < depth - 1; i++) {
            sprintf(bin_tree_paths[*returnSize], "%s%d->", bin_tree_paths[*returnSize], path[i]);//每次循环打印之前的字符串与下一个节点并加上箭头
        }
        sprintf(bin_tree_paths[*returnSize], "%s%d", bin_tree_paths[*returnSize], path[i]);//最后一个值不需要箭头
        (*returnSize)++;
    }
    if (root->left != NULL) {
